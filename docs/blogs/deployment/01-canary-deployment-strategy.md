...
도메인 서비스들의 **Clean Architecture 리팩토링**을 앞두고 있다. Auth 서비스를 시작으로 모든 도메인 서비스에 다음 변경이 예정되어 있다:
...
이는 단순한 버그 픽스가 아니라 **코드베이스의 근본적인 변경**이다. 프로덕션에 직접 배포하기엔 위험이 크다.
...
| **리소스 효율** | 추가 리소스 최소화 (`maxSurge: 1`이면 1개만 추가) |
...
Clean Architecture 리팩토링은 내부 구조가 완전히 바뀌지만 API는 동일해야 한다. 만약 리팩토링 과정에서 **의도치 않은 동작 변경**이 발생하면:
...
| 기준 | 가중치 | Rolling | Blue-Green | Canary | 비고 |
|------|:------:|:-------:|:----------:|:------:|------|
...
우리 클러스터에는 이미 **Istio Service Mesh**가 구축되어 있다:
...
| 서비스 | 네임스페이스 | VirtualService | DestinationRule | Canary Deploy |
|--------|-------------|:--------------:|:---------------:|:-------------:|
| auth-api | auth | ✅ | ✅ (신규) | ✅ |
...
1. **안전한 리팩토링**: 실제 프로덕션 환경에서 검증 후 전환
